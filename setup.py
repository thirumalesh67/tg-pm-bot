...
class DB:
    ...
    def create_job(self, JobName, GroupId, GroupName, Message):
        self.cursor.execute(f"""
            ...
            VALUES ('{JobName}', {GroupId}, '{GroupName}', '{Message}', '{datetime.now().strftime("%B %d, %Y %I:%M%p")}', '{datetime.now().strftime("%B %d, %Y %I:%M%p")}' )
        """)
    ...
    def update_job_details_user_status(self, jd_id, status):
        self.conn.execute(f"""
            UPDATE JobDetails SET Status = '{status}' WHERE ID = {jd_id}
        """)
        self.conn.commit()
    ...
    def create_group(self, GroupId, GroupTitle, GroupUserName, GroupAccessHash):
        self.cursor.execute(f"""
            ...
            VALUES ('{GroupId}', '{GroupTitle}', '{GroupUserName}', '{GroupAccessHash}', '{datetime.now().strftime("%B %d, %Y %I:%M%p")}', '{datetime.now().strftime("%B %d, %Y %I:%M%p")}')
        """)
        inserted_id = self.cursor.lastrowid
        self.conn.commit()
        return inserted_id
    ...
    def create_group_users(self, groupId,groupTitle,users):
        for user in users:
            self.conn.execute(f"""
                ...
                VALUES ({groupId}, '{groupTitle}', '{user.id}', '{user.username}', '{user.access_hash}', '{user.first_name}', '{user.last_name}', '{user.phone}', '{datetime.now().strftime("%B %d, %Y %I:%M%p")}', '{datetime.now().strftime("%B %d, %Y %I:%M%p")}')
            """)
        self.conn.commit()
